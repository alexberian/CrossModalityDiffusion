...
#----------------------------------------------------------------------------
# Cross domain version of the SongUNet model.

@persistence.persistent_class
class CrossDomainSongUNet(SongUNet):
    ...
    def __init__(self,
        ...
        domain_dim          = None,            # Length of the domain embedding.
    ):
        ...
        # set up the mapping for the domain embedding
        if domain_dim is not None:
            self.map_domain = Linear(in_features=domain_dim, out_features=self.noise_channels, init_mode='xavier_normal')
        else:
            self.map_domain = None

    def forward(self, x, noise_labels, class_labels, augment_labels=None, domain_embedding=None):
        # Mapping.
        emb = self.map_noise(noise_labels)
        emb = emb.reshape(emb.shape[0], 2, -1).flip(1).reshape(*emb.shape) # swap sin/cos
        if self.map_label is not None:
        ...
        if self.map_augment is not None and augment_labels is not None:
            emb = emb + self.map_augment(augment_labels)
        if self.map_domain is not None:
            emb = emb + self.map_domain(domain_embedding)
        if self.map_domain is not None: # add domain embedding
            emb = emb + self.map_domain(domain_embedding)
        emb = silu(self.map_layer0(emb))
        emb = silu(self.map_layer1(emb))
