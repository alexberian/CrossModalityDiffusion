...
# script for comparing using input views individually vs fusing them together
...
@click.option("--use_input_indices", help="Out of num_input_views, which ones to actually use", required=True, type=str)  # split by commas
@click.option("--num_input_views", help="How many input views to use", required=True)

def main(**kwargs):
    kwargs["input_domains"] = kwargs["input_domains"].split(",")
    kwargs["target_domains"] = kwargs["target_domains"].split(",")
    use_input_indices = [int(x) for x in kwargs["use_input_indices"].split(",")]
    use_input_indices = torch.tensor(use_input_indices)
    num_input_views = int(kwargs["num_input_views"])
    ...
    with torch.no_grad():
        for i in range(len(dataset)):
            ...
            # only use a subset of input views for generating target views
            inputs_encoded = (inputs_encoded[0][:, use_input_indices], inputs_encoded[1][:, use_input_indices])
