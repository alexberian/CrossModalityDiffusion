...
def format_CD_srncars_data(data):
    """
    Using torch's dataloader on CrossDomainNVS data class in cross_domain_srncars.py yields
    weirdly formatted batches. Also to achieve uniformity across a batch, the number of input
    images has to be fixed at 3 even though we should be training on 1-3.
    This function reformats the batch coming out of dataloader and randomly chooses the number
    of input views from 1-3.

    Args:
        data (dict): a batch of data coming out of dataloader for CrossDomainNVS data class
    
    Returns:
        data (dict): same batch but formatted better and has random number of input views
    """
    # data format that torch's dataloader produces is not good
    # reformat it
    # dataloader needs same num_input_views in __getitem__ method so we weren't able to set it randomly,
    # choose number of input views randomly here
    ...
    # 1. randomly choose number of input views to use
    num_input_views = torch.randint(1, 3+1, (1,)).item()  # 1 to 3

    # 2. format input and target domain info to match shape comment above
    ...
    # 3. truncate to num_input_views
    input_imgs = input_imgs[:, :num_input_views, ...]
    input_poses = input_poses[:, :num_input_views, ...]

    # 4. save new format
    data["input_images"] = input_imgs
    data["input_domain_info"] = input_domain_info
    data["target_domain_info"] = target_domain_info
    data["input_poses"] = input_poses
